...
# Page configuration
st.set_page_config(
    page_title="BEED Department - Training Needs Clustering Analysis",
    page_icon="📊",
    layout="wide",
...
# Main app
def main():
    st.title("📊 Training Needs Clustering Analysis")
...
def show_dashboard(df):
    ...
    # Rating scale info
    st.info("📊 **Rating Scale:** 1 = No Need, 2 = Low Need, 3 = Moderate Need, 4 = High Need, 5 = Urgent Need. Higher scores indicate higher training needs.")
...
def show_self_assessment(df):
    ...
    with st.form("self_assessment_form"):
        ...
        for domain_num in sorted(domain_mapping.keys(), key=int):
            ...
            with st.expander(f"🔰 {domain_num}. {domain_name}", expanded=False):
                for comp_col in domain_cols_list:
                    ...
                    rating = st.radio(
                        "",
                        options=[1, 2, 3, 4, 5],
                        index=2,  # Default to 3 (Moderate Need)
